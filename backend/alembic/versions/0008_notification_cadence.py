...
def upgrade():
    op.add_column('notifications', sa.Column('last_sent_at', sa.DateTime(), nullable=True))
    op.add_column('notifications', sa.Column('next_send_at', sa.DateTime(), nullable=True))
    # Prevent duplicate active notifications of same type for same company/payment
    op.create_index('ix_notifications_unique_active', 'notifications', ['company_code','payment_id','type','status'], unique=False)
