...
export default function AccountantNotifyScreen() {
    ...
    const fetchApprovalData = async () => {
        try {
            ...
            const response = await fetch(`${process.env.EXPO_PUBLIC_APP_URI}/accountant/payments/pending`, {
                method: 'GET',
                headers: {
    ...
    const handleApprove = (item) => {
        ...
        (async () => {
            ...
            try {
                ...
                const response = await fetch(`${process.env.EXPO_PUBLIC_APP_URI}/accountant/payments/${item.id}/approve`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const handleModalSubmit = async () => {
        ...
        try {
            ...
            // For accountant endpoints we use the /accountant prefix. On reject we send a
            // decline request with the comment as a query param. On approve we call the
            // approve endpoint.
            let url = '';
            if (modalAction === 'approve') {
                url = `${process.env.EXPO_PUBLIC_APP_URI}/accountant/payments/${modalItem.id}/approve`;
            } else {
                const comment = encodeURIComponent(modalComment.trim() || '');
                url = `${process.env.EXPO_PUBLIC_APP_URI}/accountant/payments/${modalItem.id}/decline?comment=${comment}`;
            }
